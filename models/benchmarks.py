...
class GPR_prop(MessagePassing):
    ...
    def __init__(self, K, alpha, Init, beta=None, bias=True, **kwargs):
        ...
        if Init == 'SGC':
            ...
            TEMP[-1] = (1 - alpha) ** K  #TEMP 表示初始化的 GPR 权重数组
        elif Init == 'NPPR':
            # Negative PPR
        ...
        self.temp = Parameter(torch.tensor(TEMP)) #GPR 权重
    ...
    def forward(self, x, edge_index, edge_weight=None):
        ...
        for k in range(self.K):
            ...
            beta = self.temp[k + 1] #GPR 权重传给了beta
    ...
    def message(self, x_j, norm):
        ...
        return norm.view(-1, 1) * x_j #使用了 norm（即邻接矩阵的标准化结果）来加权传播给定节点的信息，x_j 是邻居节点的特征
...
class GPRGNN(torch.nn.Module):
    def __init__(self, dataset, args):
        ...
        if args.ppnp == 'PPNP':
            self.prop1 = APPNP(args.K, args.alpha)
            # 如果我们将GPR-GNN的weights固定不动为PPR weights的形式，则将GPR-GNN还原为APPNP,
            # 因为它的所有GPR weights都是正值，所以它不可避免地成为了一个低通滤波器；
        elif args.ppnp == 'GPR_prop':
            self.prop1 = GPR_prop(args.K, args.alpha, args.Init, args.beta)
